...
常规视图显示五小时和本周剩余额度及各自重置时间；备用视图显示备用剩余额度和常规额度恢复时间。图中数值与时间仅作示例。

## 行为

- 仅在 Codex 窗口位于前台且没有最小化时显示，不会浮在其他应用上方。
- 以约 4ms 的定位节奏随 Codex 窗口移动、最小化和恢复；运行期间使用
  1ms 高精度计时，退出时自动释放。
...
- 常规额度耗尽后改为两行备用视图：备用剩余额度、常规额度恢复时间。
  从独立 `base_model_inference` 桶读取备用额度；接口未提供时显示
  `备用 未提供 / 接口未返回`，仍保留常规恢复时间，不沿用旧百分比。
  备用百分比存在但未提供重置时间时，仍正常显示百分比。
